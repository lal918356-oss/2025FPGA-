...
//*********************************************************************
// 选择工作模式：
// - 发送端模式：取消注释 `define FPGA_SENDER 行
// - 接收端模式：取消注释 `define FPGA_RECEIVER 行
// - 同时只能选择一种模式
//*********************************************************************
//`define FPGA_SENDER    // FPGA发送端模式
`define FPGA_RECEIVER  // FPGA接收端模式
...
`ifdef FPGA_SENDER
  // 发送端模式 - 不使用回环
`else
  `define UDP_LOOP_BACK  // 接收端模式 - 使用回环
`endif
...
udp_data_tpg u1_udp_data_tpg(
    ...
    .tpg_data_enable    (phy_reset          ),// 当物理层就绪后开始发送数据
    .tpg_data_header0   (16'haabb           ),//帧头0
    .tpg_data_header1   (16'hccdd           ),//帧头1
    .tpg_data_type      (16'ha8b8           ),//数据帧类型
    .tpg_data_length    (16'h00ff           ),//数据长度255字节
    .tpg_data_num       (16'h000a           ),//每次发送10个数据包
